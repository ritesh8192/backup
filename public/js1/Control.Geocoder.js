...
this.L.Control.Geocoder = (function (L) {
  ...
  var ArcGis = L.Class.extend({
    ...
    geocode: function(query, cb, context) {
      ...
      getJSON(
        this.options.service_url + '/findAddressFreelancers',
        ...
        function(data) {
          ...
          if (data.freelancers && data.freelancers.length) {
            for (var i = 0; i <= data.freelancers.length - 1; i++) {
              loc = data.freelancers[i];
  ...
  var LatLng = L.Class.extend({
    ...
    geocode: function(query, cb, context) {
      ...
      if ((match = query.match(/^([NS])\s*(\d{1,3}(?:\.\d*)?)\W*([EW])\s*(\d{1,3}(?:\.\d*)?)$/))) {
        ...
        (match = query.match(
          /^([NS])\s*(\d{1,3})Â°?\s*(\d{1,3}(?:\.\d*)?)?['â€²]?\W*([EW])\s*(\d{1,3})Â°?\s*(\d{1,3}(?:\.\d*)?)?['â€²]?$/
        ))
      ) {
        ...
        (match = query.match(
          /^(\d{1,3})Â°?\s*(\d{1,3}(?:\.\d*)?)?['â€²]?\s*([NS])\W*(\d{1,3})Â°?\s*(\d{1,3}(?:\.\d*)?)?['â€²]?\s*([EW])$/
        ))
      ) {
        ...
        (match = query.match(
          /^([NS])\s*(\d{1,3})Â°?\s*(\d{1,2})['â€²]?\s*(\d{1,3}(?:\.\d*)?)?["â€³]?\W*([EW])\s*(\d{1,3})Â°?\s*(\d{1,2})['â€²]?\s*(\d{1,3}(?:\.\d*)?)?["â€³]?$/
        ))
      ) {
        ...
        (match = query.match(
          /^(\d{1,3})Â°?\s*(\d{1,2})['â€²]?\s*(\d{1,3}(?:\.\d*)?)?["â€³]\s*([NS])\W*(\d{1,3})Â°?\s*(\d{1,2})['â€²]?\s*(\d{1,3}(?:\.\d*)?)?["â€³]?\s*([EW])$/
        ))
      ) {
